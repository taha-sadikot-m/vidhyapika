...
import { createTopic } from "../../../../../../../backend/repositories/curriculumRepo";
import { z } from "zod";
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      try {
        const id = await createTopic({
          ...
          finalTestThreshold: parsed.data.finalTestThreshold,
        } as any);
        created.push({ index: i, id });
      } catch (e: any) {
        errors.push({ index: i, message: e?.message ?? "Failed to create topic" });
      }
    }
